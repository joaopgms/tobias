...
class GitHubStore:
    ...
    def append_report(self, report_type: str, entry: dict,
                       max_rolling: int = 30) -> None:
        """Append a report entry to both the rolling .js and the archive .json."""
        import json as _json
        now_ts = entry.get("ts", "")
        js_key = f"{report_type}_reports_js"
        try:
            existing_js, _ = self._get_file(self.FILES[js_key])
            existing_js = existing_js.strip()
            if existing_js.startswith("const"):
                inner = existing_js[existing_js.find("["): existing_js.rfind("]")+1]
                reports = _json.loads(inner) if inner.strip() not in ("", "[]") else []
            else:
                reports = _json.loads(existing_js) if existing_js else []
        except Exception:
            reports = []
        reports.append(entry)
        if len(reports) > max_rolling:
            reports = reports[-max_rolling:]
        var_name = "SCOUT_REPORTS" if report_type == "scout" else "COMMIT_REPORTS"
        js_content = (
            f"// Auto-generated by Tobias -- do not edit manually\n"
            f"// Last updated: {now_ts}\n"
            f"const {var_name} = {_json.dumps(reports, indent=2, ensure_ascii=False)};\n"
        )
        self._put_file(self.FILES[js_key], js_content,
                       f"{report_type}: report {now_ts[:10]}")
