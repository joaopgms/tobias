...
- Austin Reaves (G): OUT [roster-only] — key secondary player; LAL depth thin yet dominant.
NOTE: Do NOT bet LAL spread at short odds in closing game — blowout and garbage-time risk.
...
- Anthony Edwards (G): OUT [roster-only] — FRANCHISE PLAYER. STATUS ALERT: Edwards appeared active in MIN leading DEN 3-2 per ESPN; re-verify Edwards status vs NBA official PDF before ANY MIN pick. Also re-verify Rudy Gobert, Mike Conley, Julius Randle.
...
- Shai Gilgeous-Alexander, Chet Holmgren (OKC) — re-verify each session
...
- Anthony Edwards (MIN) — was roster-only OUT; may be active per ESPN series data — re-verify this session CRITICAL
